...
m = Map (modulename, translate("Call Routing"),
         translate("This is where you indicate which SIP accounts are used to call what \
                   country/area codes, which users can use what SIP accounts, how incoming \
...
if     nallvalidaccounts == 0 then
   text = translate("NOTE: There are no SIP provider accounts configured.")
elseif nvalidoutaccounts == 0 then
   text = translate("NOTE: There are or SIP provider accounts enabled for outgoing calls.")
else
...
elseif nvalidinaccounts == 0 then
   text = translate("NOTE: There are no SIP provider accounts enabled for incoming calls.")
else
